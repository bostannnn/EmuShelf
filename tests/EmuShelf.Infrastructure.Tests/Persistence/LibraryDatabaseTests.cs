...
public class LibraryDatabaseTests : TempAppDirectoryTestBase
{
    ...
    public void Initialize_CreatesExpectedTables()
    {
        ...
        Assert.Contains("GameDiscSelections", GetTableNames(database));
    ...
    public void Initialize_AddsPlaytimeColumns_DefaultingToZero()
    {
        ...
        using var connection = database.CreateConnection();
        InsertGame(connection, "PS1/g.cue");
        using var command = connection.CreateCommand();
        command.CommandText = "SELECT PlaytimeSeconds, PlayCount FROM Games LIMIT 1;";
    ...
    public void Initialize_FromVersion2_PreservesExistingCoverAsUserOwned()
    {
        ...
        Assert.Equal(20L, (long)version.ExecuteScalar()!);
