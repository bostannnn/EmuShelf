...
public sealed class LoopbackOAuthRedirectHandlerTests
{
    ...
    [Fact]
    public async Task WaitForCode_WithAnAlreadyCancelledToken_ThrowsCancellationNotAFailure()
    {
        // A token cancelled before the wait starts must not surface as "connect failed" — Stop() runs
        // first and the accept then throws InvalidOperationException, which used to escape uncaught.
        using var handler = new LoopbackOAuthRedirectHandler();
        using var cancellation = new CancellationTokenSource();
        await cancellation.CancelAsync();

        await Assert.ThrowsAnyAsync<OperationCanceledException>(
            () => handler.WaitForCodeAsync("the-state", cancellation.Token));
    }
