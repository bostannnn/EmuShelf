...
public sealed class GoogleDriveCloudSyncTransportTests : TempAppDirectoryTestBase
{
    ...
    [Fact]
    public async Task Download_ResolvesTheOldestBlobWhenAFolderHasDuplicateNames()
    {
        // Drive permits two files with the same name, and two machines' first uploads of one unit can
        // create exactly that. Both machines must then resolve the unit to the SAME blob regardless of
        // the order Drive lists them in, or they never converge — nothing is ever deleted. Oldest-wins
        // is the tiebreak, matching FindChildAsync.
        var drive = new FakeDriveServer();
        drive.AddFile(
            $"{CloudFolder}/{CloudSaveIndex.FileName}",
    ...
    public async Task Download_FindsUnitsAcrossDuplicateProviderFolders_NotOnlyTheOldestFolder()
    {
        ...
        // The unit that lives only in the newer duplicate folder is still reachable.
        Assert.Equal("in-newer", await readerB.ReadToEndAsync(Cancellation));
    }
