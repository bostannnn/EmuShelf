...
**First, correct the status.** Phases 1–2 are *not* done: they are one unmerged 28-file / 4,928-line
commit on `claude/android-port-feasibility-6013e5`, never on `main`, which `main` has since moved 12
commits past. Nothing has touched Google's real API. Before this counts as groundwork: merge it split
into reviewable commits, ship the desktop settings UI, and make one real sign-in.

**Remaining, desktop (parallel, and worth doing regardless):** the settings UI — transport choice,
connect/disconnect, and the switch-time warning that an rclone-created folder is invisible under
`drive.file` so saves re-upload.
...
3. Force `TransportKind` to `GoogleDrive` — and note that "hide the rclone UI" is a **real rebuild of
   the gamepad Saves section**, which is rclone-shaped end to end (67 references across the
   coordinator and both settings view-models, plus `CloudRemoteName` in Core).
