...
Status: Phases 1–2 built (2026-08-15). Supersedes nothing yet; see "Relationship to the decision log".
...
`CloudSaveSyncCoordinator`, a managed connect flow beside `ConnectGoogleDriveAsync`, and the
switch-time warning from finding 5. Desktop and Gamepad settings gain the choice; rclone's install
prompt stops being the default path.
