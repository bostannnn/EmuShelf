        ...
        _ = Task.Run(async () =>
        {
            ...
            async Task PublishAsync(CompanionMediaSet media)
            {
                ...
                await global::Avalonia.Threading.Dispatcher.UIThread.InvokeAsync(() =>
                {
                    ...
                    // Pre-PR-244 entrance: the wordmark/title shows only when the game has neither fan art
                    // nor logo — never painted over fan art. (SetSpotlight's own wheel-is-null default only
                    // fires for the both-null clear paths, so it stays consistent with this.)
                    model.ShowBranding = background is null && logo is null;
