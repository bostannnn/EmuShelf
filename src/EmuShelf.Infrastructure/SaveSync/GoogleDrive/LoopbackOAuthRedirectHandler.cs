...
public sealed class LoopbackOAuthRedirectHandler : IOAuthRedirectHandler
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        while (true)
        {
            // Covers a token already cancelled on entry, and one cancelled in the gap after answering
            // a favicon/prefetch and before the next accept — neither reaches GetContextAsync, so
            // neither would surface as the HttpListenerException the Stop() path relies on.
            ...
            {
                // Cancellation raced Stop(): GetContextAsync on a stopped-but-not-disposed listener
                // throws this. It is a cancellation, not a connect failure.
                throw new OperationCanceledException(cancellationToken);
            }
