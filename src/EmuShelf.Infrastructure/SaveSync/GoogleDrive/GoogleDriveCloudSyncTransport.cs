...
public sealed class GoogleDriveCloudSyncTransport : IVerifiableCloudSyncTransport
{
    ...
    private async Task LoadTreeAsync(CancellationToken cancellationToken)
    {
        ...
        while (pending.Count > 0)
        {
            ...
            foreach (var child in await _api.ListChildrenAsync(id, cancellationToken))
            {
                var childPath = path.Length == 0 ? child.Name : path + "/" + child.Name;
                if (child.IsFolder)
                {
                    _folderIds[childPath] = child.Id;
                    pending.Enqueue((childPath, child.Id));
                }
                else
                {
                    // First writer wins on a duplicate name, matching FindChildAsync's oldest-wins
                    // rule, so the two never disagree about which blob a unit means.
